...
import { useGetCourse } from "@/hooks/user/course.hooks";
    ...
    <div className="space-y-4">
              ...
              <button key={key} className="w-1/3" onClick={() => router.push(`/course/${obj.course.id}/learn`)}>
                ...
                <CardBody className="overflow-visible py-2">
                  <div className="flex justify-between w-full items-center">
                    ...
                    <Chip size="sm">{obj.course.category ? obj.course?.category.name : 'Category'}</Chip>
                  </div>
                </CardBody>
                <CardFooter>
      ...
      {!isFetching && data?.result.length !== 0 &&
        <div className="flex justify-between items-center">
          <p>Show {data.pagination.total_data} data from {data.pagination.total_page}</p>
          <Pagination
            loop
            showControls
            page={currentPage}
